...
import { AvatarGallery, AvatarDisplay } from "./avatars/AvatarGallery";
...
interface OnlineLobbyProps {
  ...
  onJoinRoom: (code: string, name: string, avatarId: string) => void;
  onLeaveRoom: () => void;
...
export const OnlineLobby: React.FC<OnlineLobbyProps> = ({
  ...
  const [hostAvatarId, setHostAvatarId] = useState(getRandomAvatar("male").id);
  const [joinCode, setJoinCode] = useState("");
  const [joinName, setJoinName] = useState("");
  const [joinAvatarId, setJoinAvatarId] = useState(getRandomAvatar("female").id);
  ...
  const handleCreate = () => {
    ...
    setFormError(null);
    onCreateRoom(hostName, hostAvatarId);
  };
  ...
  const handleJoin = () => {
    ...
    setFormError(null);
    onJoinRoom(joinCode, joinName, joinAvatarId);
  };
      ...
      <div className="w-full max-w-5xl mx-auto">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          ...
          <div className="text-center">
            <p className="text-sm font-bold uppercase tracking-[0.4em] bg-neo-black text-neo-white inline-block px-2 py-1">
              Online Rooms
            </p>
        ...
        {!isConnected && (
          <div className="mb-8 bg-neo-yellow border-4 border-neo-black p-4 text-center shadow-neo-sm animate-pulse">
            ...
            <p className="text-sm font-neo-body mt-2 font-bold">Multiplayer requires the backend server.</p>
            <p className="text-xs mt-2">
              Run <code className="bg-neo-black text-neo-white px-2 py-1 font-mono">npm run dev:full</code> to start
              both.
            </p>
          </div>
        )}
        ...
        {!roomCode && (
          <div className="grid gap-8 md:grid-cols-2">
            <div className="neo-card relative flex flex-col h-full">
              ...
              <div className="pt-8 flex-1 flex flex-col">
                <h2 className="text-2xl font-neo-display font-bold uppercase">Create a room</h2>
                <p className="font-neo-body text-sm mt-2 mb-6 border-b-2 border-neo-black pb-4">
                  Share the invite code with a friend.
                </p>
                ...
                <div className="mt-auto">
                  <label className="block text-sm font-bold uppercase mb-2">Your avatar</label>
                  <AvatarGallery
                    selectedAvatarId={hostAvatarId}
                    onAvatarSelect={setHostAvatarId}
                    title=""
                    defaultGender="male"
                  />
                </div>
              ...
              <button
                ...
                className={`mt-4 bg-neo-green border-4 border-neo-black shadow-neo-lg w-full py-4 text-2xl font-neo-display font-bold uppercase tracking-wider transition-all transform hover:scale-105 hover:bg-neo-black hover:text-neo-green active:translate-x-[4px] active:translate-y-[4px] active:shadow-none ${
            ...
            <div className="neo-card relative flex flex-col h-full">
              ...
              <div className="pt-8 flex-1 flex flex-col">
                <h2 className="text-2xl font-neo-display font-bold uppercase">Join with code</h2>
                <p className="font-neo-body text-sm mt-2 mb-6 border-b-2 border-neo-black pb-4">
                ...
                <div className="mt-auto">
                  <label className="block text-sm font-bold uppercase mb-2">Your avatar</label>
                  <AvatarGallery
                    selectedAvatarId={joinAvatarId}
                    onAvatarSelect={setJoinAvatarId}
                    title=""
              ...
              <button
                onClick={handleJoin}
                disabled={!isConnected}
                className={`mt-4 bg-neo-blue border-4 border-neo-black shadow-neo-lg w-full py-4 text-2xl font-neo-display font-bold uppercase tracking-wider transition-all transform hover:scale-105 hover:bg-neo-black hover:text-neo-blue active:translate-x-[4px] active:translate-y-[4px] active:shadow-none ${
                  !isConnected ? "opacity-50 cursor-not-allowed" : ""
                }`}
              >
          ...
          <div className="neo-card">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b-4 border-neo-black pb-6 mb-6">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <span className="text-4xl font-mono font-bold tracking-[0.2em] bg-neo-yellow border-2 border-neo-black px-4 py-2 shadow-neo-sm">
                    {roomCode}
                  </span>
                  <button
                    className="text-sm font-bold uppercase hover:bg-neo-black hover:text-neo-white border-2 border-neo-black px-4 py-2 transition-all transform hover:scale-105 active:translate-x-[2px] active:translate-y-[2px] active:shadow-neo-sm"
              ...
              <button
                onClick={onLeaveRoom}
                className="text-sm font-bold uppercase text-neo-red hover:bg-neo-red hover:text-neo-white border-4 border-neo-red px-6 py-3 shadow-neo-sm transition-all transform hover:scale-105 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
              >
                LEAVE ROOM
              </button>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-8">
              {players.map((player) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-100 border-2 border-neo-black flex items-center justify-center">
                      <AvatarDisplay avatarId={player.avatarId || "explorer"} size="small" />
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase tracking-widest mb-1 bg-neo-black text-neo-white inline-block px-1">
            ...
            {playerRole === "host" ? (
              ...
              <p className="text-center font-bold uppercase animate-pulse border-t-4 border-neo-black pt-6">
                Waiting for the host to configure the route…
              </p>
            )}
            ...
            {playerRole === "host" && (
              <button
                ...
                className={`w-full py-6 font-neo-display font-bold uppercase text-3xl border-4 border-neo-black shadow-neo-lg transition-all transform hover:scale-105 ${
                  canLaunch
                    ? "bg-neo-red text-neo-white hover:bg-neo-black hover:text-neo-red active:translate-x-[4px] active:translate-y-[4px] active:shadow-none"
