...
import { connectSocket, disconnectSocket, clearSocketInstance } from './services/socketService';
import { GameState, GameMode, Location, TriviaQuestion, LobbyPlayer, QuestionRevealState } from './types';
...
const App: React.FC = () => {
  ...
  const [questionReveal, setQuestionReveal] = useState<QuestionRevealState | null>(null);
  const [questionQueue, setQuestionQueue] = useState<Map<string, TriviaQuestion[]>>(new Map());
  ...
  const preloadQuestions = useCallback(async (
    ...
    locationsNeedingFetch.forEach((name, index) => {
      ...
      if (result.status === 'fulfilled' && Array.isArray(result.value)) {
        ...
        const uniqueNewQuestions = result.value.filter(nq => !existing.some(eq => eq.question === nq.question));
  ...
  const handleStartGame = useCallback(async (start: Location, end: Location, p1Name: string, p2Name: string) => {
    setIsLoading(true);
    ...
    const newQueue = await preloadQuestions(newPath, 0, 0, new Map(), new Set());
  ...
  const handleReadyForQuestion = async () => {
    ...
    let unusedQuestions = availableQuestions.filter(q => !usedQuestions.has(q.question));
    ...
    if (unusedQuestions.length === 0) {
      ...
      setQuestionQueue(prevQueue => {
        ...
        const uniqueNewQuestions = newQuestions.filter(nq => !existing.some(eq => eq.question === nq.question));
        updatedQueue.set(location.name, [...existing, ...uniqueNewQuestions]);
        return updatedQueue;
      });

      unusedQuestions = newQuestions.filter(q => !usedQuestions.has(q.question));

      if (unusedQuestions.length === 0) {
        alert(`Could not find a unique question for ${location.name}. Skipping turn.`);
        handleAnswer(false);
        return;
      }
    }
  ...
  const handleAnswer = (isCorrect: boolean) => {
    ...
    const answeringPosition = currentPlayer === 1 ? player1Position : player2Position;
    const destinationIndex = Math.max(racePath.length - 1, 0);
    const isAnsweringAtDestination = racePath.length > 0 && answeringPosition === destinationIndex;
    ...
    if (!newWinner) {
      ...
      preloadQuestions(racePath, nextP1Pos, nextP2Pos, questionQueue, usedQuestions).then(setQuestionQueue);
    }
  };

  const handleGetMoreInfo = async (question: string, answer: string) => {
    if (gameMode === 'online' && !isHost) {
      socket?.emit('client:action', { type: 'moreInfo', payload: { question, answer } });
      return;
    }
  ...
  const handleRemoteReveal = useCallback((state: QuestionRevealState) => {
  ...
  const handleQuestionReveal = useCallback((state: Omit<QuestionRevealState, 'questionId'>) => {
  ...
  useEffect(() => {
    ...
    const handleRoomCreated = ({ code, player }: { code: string; player: LobbyPlayer }) => {
      setRoomCode(code);
      setPlayerRole('host');
      setLobbyPlayers([player]);
      setLobbyStatus('Room live! Share the code with a friend.');
    };
    const handleRoomJoined = ({ code }: { code: string }) => {
      setRoomCode(code);
      setPlayerRole('guest');
      setLobbyStatus('Joined room. Waiting for the host to start.');
      setGameState('onlineLobby');
    };
    const handleRoomUpdate = ({ players }: { code: string; players: LobbyPlayer[] }) => {
    ...
    const handleRoomAction = ({ payload }: { from: string; payload: { type: string; payload?: any } }) => {
  ...
  useEffect(() => {
    if (gameMode !== 'online' || !isHost || !socket || !socket.connected || !roomCode) return;
  ...
  const renderGameState = () => {
    ...
    if (gameState === 'setup' && gameMode === 'local') {
      return <GameSetup onStartGame={handleStartGame} onBack={handleReturnToLanding} />;
    }
